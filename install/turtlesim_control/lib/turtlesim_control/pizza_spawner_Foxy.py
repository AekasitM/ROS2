...
def read_yaml_file(path:str):    #read yaml file to build path
...
class DummyNode(Node):  #Node to spawn pizza
    def __init__(self):
        super().__init__('pizza_spawner_Foxy')
    ...
    def set_goal_callback(self, request:SendPosition.Request, response:SendPosition.Response):#Set goal service
    ...
    def turtle_pose_callback(self, msg): #check position now
    ...
    def pizza_path(self,count):     #path to spawn pizza
        if self.count < 20:
            self.turtle_target_pose[0] = values_list_Foxy[0][count][0]
            self.turtle_target_pose[1] = values_list_Foxy[0][count][1]
            self.count = count+1
    ...
    def controller(self):       # to find path for spawning pizza
